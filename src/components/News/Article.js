...
class Article extends React.Component {
  ...
  render() {
    const { imageurl, title, body, source_info: { name }, published_on: publishedOn, categories, url } = this.props;
    const timeFromNow = moment.unix(publishedOn).fromNow();
    const categoriesList = categories.split('|').reduce((prev, curr) => [prev, <span key={prev} className="news__category"> | </span>, curr])
    const imageTitle = `See the full article from ${name}`;
    let parsedBody = parse(body);
    if (parsedBody.length > 200)
      parsedBody = parsedBody.slice(0, 200) + ' ...';
